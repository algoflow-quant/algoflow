...
export function CustomNavbarDropdownButton({href, icon, title, description, iconBgClassName}: CustomNavbarDropdownButtonProps) {
  return (
    <Link href={href} className="flex flex-row gap-3 group p-2 rounded-sm transition-all group">
        {/* Icon container */}
        <div className={`w-10 h-10 shrink-0 rounded-md border flex items-center justify-center ${iconBgClassName || 'bg-muted'}`}>
            {icon}
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center">
            <div className="flex items-center gap-2">
                ...
                <ChevronRight style={{ width: '14px', height: '13px' }} className="opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200" />
            </div>
...
export function CustomNavbarDropdownButtonAlternative({href, icon, title, description, iconBgClassName}: CustomNavbarDropdownButtonProps) {
  return (
    <Link href={href} className="flex flex-row gap-3 group p-2 rounded-sm transition-all group">
        {/* Icon container */}
        <div className={`w-8 h-8 shrink-0 rounded-md border flex items-center justify-center ${iconBgClassName || 'bg-transparent'}`}>
            {icon}
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center -mt-1">
            <div className="flex items-center gap-2">
                ...
                {/* Chevron - hidden by default, slides in on hover */}
                <ChevronRight style={{ width: '13px', height: '12px' }} className="opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200" />
            </div>
