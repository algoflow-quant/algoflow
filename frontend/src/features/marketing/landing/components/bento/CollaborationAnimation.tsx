...
export default function CollaborationAnimation() {
  ...
  useEffect(() => {
    if (!isHovered) {
      setShowCode(false)
      setActiveCursors([])
      setClickCursor({ x: 0, y: 0, show: false })
    ...
    mikeText.split('').forEach((_, i) => {
      editSequence.push({
        ...
        action: () => {
          ...
          setCodeLines(prev => {
            const newLines = [...prev]
            newLines[6] = '    signals if sma_20 > sma_50' + typedText
            return newLines
          })
        }
      })
  ...
  return (
    <div
      className="absolute inset-0 flex flex-col p-4 pb-24"
